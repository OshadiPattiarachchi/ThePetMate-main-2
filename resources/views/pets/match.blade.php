...
<nav class="navbar">
    ...
    <div class="container">
        <h2 class="text-center my-4">Matching Pets for {{ $pet->name }}</h2>
            ...
            <div class="row">
                @foreach($matchedPets as $match)
                    <div class="col-md-4 mb-4">
                        <div class="card">
                            <!-- Display Pet Image -->
                            <img src="{{ asset('storage/' . $match->photo) }}" class="card-img-top" alt="{{ $match->name }}" onerror="this.onerror=null;this.src='https://via.placeholder.com/150';">
                            <div class="card-body">
                                <!-- Pet Details -->
                                <h5 class="card-title">{{ $match->name }}</h5>
                                <p class="card-text">
                                    <strong>Type:</strong> {{ $match->type }}<br>
                                    <strong>Breed:</strong> {{ $match->breed }}<br>
                                    <strong>Gender:</strong> {{ ucfirst($match->gender) }}<br>
                                    <strong>Age:</strong> {{ $match->age }} years<br>
                                    <strong>City:</strong> {{ $match->city }}<br>
                                </p>
                                <!-- View Match Button -->
                                <a href="{{ route('pets.show', $match->id) }}" class="btn btn-primary">View Match Details</a>
                            </div>
                        </div>
                    </div>
                @endforeach
